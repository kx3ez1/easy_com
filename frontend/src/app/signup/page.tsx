...
function SignupContent() {
  ...
  useEffect(() => {
    ...
    if (qPassword) {
      ...
      setConfirmPassword(qPassword);
    }

    if (qEmail && qPassword && !autoSubmitted.current) {
      autoSubmitted.current = true;
      const timer = setTimeout(() => {
        performSignup(qEmail, qPassword);
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [searchParams]);

  const performSignup = async (signupEmail: string, signupPassword: string) => {
    setError(null);
    setLoading(true);

    try {
      // Proceed with Firebase Sign Up directly (instant) and trim email
      ...
      const friendlyMessage = getFriendlyErrorMessage(err.code, err.message || "Failed to create account.");
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    const confirmPasswordVal = (target.elements.namedItem("confirm-password") as HTMLInputElement)?.value || confirmPassword;

    if (passwordVal !== confirmPasswordVal) {
      setError("Passwords do not match.");
      return;
    }
    performSignup(emailVal, passwordVal);
  };

  const redirectToParam = searchParams.get("redirectTo");
  const loginLink = redirectToParam ? `/login?redirectTo=${encodeURIComponent(redirectToParam)}` : "/login";
